...
const INITIAL_CAMERA_POSITION = [20000, 10000, 20000] as const;
    ...
    return (
        <div className="relative w-full h-screen">
            <button
                ...
                className="absolute top-4 right-4 z-20 p-2 bg-black/70 text-white rounded-lg hover:bg-black/80 transition-colors sm:hidden"
                aria-label={isMenuVisible ? "Hide Menu" : "Show Menu"}
            >
                {isMenuVisible ? "✕" : "☰"}
            </button>

            <div className={`absolute top-4 left-4 p-3 sm:p-4 bg-black/70 text-white rounded-lg max-w-[calc(100%-2rem)] sm:max-w-md space-y-1.5 sm:space-y-2 z-10 text-sm sm:text-base transition-transform duration-300 ${!isMenuVisible ? '-translate-y-[120%]' : 'translate-y-0'} sm:translate-y-0`}>
                <h1 className="text-lg sm:text-xl font-bold">Pewpewlazer's Solar System at Scale</h1>
                <div className="space-y-1">
                    <p>🖱️ <span className="font-medium">Mouse Controls:</span></p>
                    <ul className="pl-3 sm:pl-4 text-sm">
                ...
                <button
                    ...
                    className="mt-1.5 sm:mt-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-white/10 hover:bg-white/20 rounded-md transition-colors w-full text-sm"
                >
                    🏠 Reset View
                </button>
                <a
                    ...
                    className="mt-1.5 sm:mt-2 flex items-center gap-2 text-white hover:text-gray-300 transition-colors text-sm"
